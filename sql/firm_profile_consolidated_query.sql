/*
  Consolidated firm profile query
  - Aggregates individual employee/member data up to the firm level
  - Uses the firm staging table as the anchor and joins employee/member facts through employers_key / individual_id
  - Leaves not-yet-structured metrics as NULL placeholders
*/
...
firm_engagement_summary AS (
    SELECT
        i.firms_key,
        MAX(ep.engagement_score) AS latest_firm_engagement_score,
        SUM(COALESCE(ep.ep_event_registration_count, 0)) AS total_engagement_event_registrations,
        SUM(COALESCE(ep.ep_committee_attendance_count, 0)) AS total_engagement_committee_attendance,
        SUM(COALESCE(ep.ep_contribution_count, 0)) AS total_engagement_contributions,
        SUM(COALESCE(ep.ep_volunteer_assignment_count, 0)) AS total_engagement_volunteer_activity,
        SUM(COALESCE(ep.ep_log_engagement_count, 0)) AS total_engagement_log_activity
